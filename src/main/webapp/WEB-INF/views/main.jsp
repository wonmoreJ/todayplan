      ...
      <table class="mainTable">
        ...
        <tbody>
          <c:forEach var="item" items="${calList}" varStatus="status">
            <tr
              id="calTr${item.year}${item.month}${item.day}"
              tabindex="${status.count}"
            >
              <td rowspan="2" class="tc calTableTd holiday${item.holidayYn}">
                ${item.fullDate}
              </td>
              <td
                rowspan="2"
                class="addBtn"
                data-num="${item.year}${item.month}${item.day}"
              >
                <c:if test="${item.dtResult == 'Y'}">
                  <button
                    type="button"
                    class="addBtnStyle addBtnHover"
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <button type="button" class="btn btn-primary" id="modalAddPlan">
                추가
              </button>
            </div>
          </div>
